...
Short context: **plants** (`name`, `spot`, optional `care_note`). **Laravel Breeze** is installed — use `<x-app-layout>`, `layouts/navigation.blade.php`, `PlantController`, `routes/web.php`. Set `APP_NAME=Garden` in `.env`. The **solution** may add “all vs my plants” / “added by” UX; ignore for marking unless your course says otherwise.
...
**Three blocks (40 points):** **1 — Listing** (`/plants` read UI, 10) · **2 — Many-to-many** (10) · **3 — Auth & create** (`/` + nav + middleware + create, 20). Order: **1 → 2 → 3**.

---

### 1 — Listing: routes, `PlantController`, Blade (10 points)

**Scope:** **`GET /plants`** listing only (read UI). Session / **`/`** home behaviour is **§3 — Home route**.

**Files:** `routes/web.php`, `PlantController`, `resources/views/plants/`. Starter already has `plants` table, seeder, `user_id` migration.

**a. (2 pts)** Register **`GET /plants`** → **`PlantController@index`**, route name **`plants.index`**.
**b. (2 pts)** In **`index()`**, load all plants; **`return view('plants.index', compact('plants'))`** (no stub).
**c. (2 pts)** **`plants/index.blade.php`:** stop using the bare placeholder wrapper. Wrap the whole page in Breeze’s **`<x-app-layout>`** Blade component (same shell as **`dashboard`**: top nav, spacing, etc.). Add **`<x-slot name="header">…</x-slot>`** so the page has a **header/title row** under the navbar (any clear title, e.g. “Plants”, is fine — same mechanism Breeze uses on other pages).
**d. (2 pts)** Render **`$plants`** (Tailwind grid/cards): **`name`**, **`spot`**, **`care_note`** when present.
**e. (2 pts)** After **`migrate:fresh --seed`**, **`/plants`** renders without error.
...
**Scope:** session-aware **`/`** route, **`layouts/navigation.blade.php`**, **`web.php`** middleware, plant **create** Blade, **`@csrf`**, **`PlantController@store`**.

#### Home route (`/`)

**a. (1 pt)** Register **`GET /`** and name the route **`home`**.
**b. (1 pt)** Authenticated user on **`/`** → **redirect** to **`dashboard`** (e.g. `route('dashboard')`).
**c. (1 pt)** Guest on **`/`** → show **`home`** with **`plantCount`** = **`Plant::count()`**.
...
**a. (3 pts)** Validate and **save** the plant (e.g. **`Plant::create`**).
**b. (3 pts)** Form field name for the spot is **`spot`**, not **`garden_spot`**.
